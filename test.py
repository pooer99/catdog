...
def predict(cvImg, model, modelPath, modelName, width, height, depth, result1, result2):
    ...
    # 加载模型
    model.load_weights("D:\Project\catdog\catdog\model\catdog_CNNweights.h5")

    # 缩放图像减少检测时间
    img = cv.resize(cvImg, (width, height))
    # 归一化
    img_arr = img / 255.0
    # 重构成模型需要输入的格式
...
img = cv.imread("D:\Project\catdog\catdog\data\\test1\\1.jpg")
cv.imshow('img', img)
predict(img,model,modelPath,"catvsdog_weights.h5",150,150,3,"猫","狗")
cv.waitKey(0)
